Public Class frmPurchaserEdit
    ...
    Sub LoadPurchase()
        Try
            ...
            Dim ff As cls_tblPurchase.Fields = objPurchase.Selection_One_Row(PurchaseId)
            txtInvoiceNo.Text = ff.InvoiceNo
            txtPO_No.Text = ff.PO_No
            txtOrderDate.Value = ff.PurchaseDate
            cmbCustomer.SelectedValue = ff.VendorId
            ...
            For Each drProduct As DataRow In dt.Rows
                dgItemList.Rows.Add(drProduct(cls_tblProducts.FieldName.ProductId.ToString), _
                                    ...
                                    drProduct(cls_tblStock.FieldName.Qty.ToString), "", "", "Delete", drProduct(cls_tblStock.FieldName.Fresh.ToString), drProduct(cls_tblStock.FieldName.Frozen.ToString), drProduct(cls_tblStock.FieldName.Remarks.ToString))
            Next
            Try
    ...
    Private Sub btnPostOrder_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPostOrder.Click
        ...
        Try
            If Val(lblTotalCases.Text) = 0 Then ' And cmbStatus.Text <> "No Order" Then
                ...
                Exit Sub
            End If
            Dim OrderId As Integer = objPurchase.Update(PurchaseId, cmbCustomer.SelectedValue, txtOrderDate.Value, txtPO_No.Text, txtInvoiceNo.Text, 0, Now, UserId, Now, UserId, Session, conn, transac)
            objStock.Delete_By_SELECT("TransactionId=" & PurchaseId.ToString & " and TransactionType='PURCHASE'", Nothing, conn, transac)
